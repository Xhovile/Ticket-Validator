...
function getCreatorAvatarUrl(creator: Record<string, unknown> | null | undefined): string | undefined {
  ...
  const candidates = [creator.profile_picture, creator.business_logo, creator.photoURL, creator.photo_url];
  const avatar = candidates.find((value) => typeof value === 'string' && value.trim().length > 0);
  return typeof avatar === 'string' ? avatar : undefined;
}

export function buildValidatorSession(response: ValidatorMeResponse): ValidatorSessionSnapshot {
  const identity = response.identity;
  const accessScope = response.access_scope;
  const authorizedEventIds = accessScope.allowed_event_ids?.length
    ? accessScope.allowed_event_ids
    : response.events.map((event) => event.id);

  const isOrganizer = Boolean(response.creator) || accessScope.role === 'admin' || Boolean(accessScope.is_admin) || Boolean(identity.is_admin);
  ...
  const events = response.events
    .map((event: ValidatorEventRecord) => mapValidatorEvent(event, identity.uid))
    .filter((event) => authorizedEventIds.includes(event.id));

  return { user, events, authorizedEventIds, isOrganizer };
}
