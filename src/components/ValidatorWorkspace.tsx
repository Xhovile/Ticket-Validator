...
import { CheckCircle2, Loader2, ShieldAlert, ScanLine } from 'lucide-react';
...
export function ValidatorWorkspace({ controller }: { controller: ValidatorController }) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-white px-6 text-black">
        <div className="flex flex-col items-center gap-4 text-center">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#020617] text-white shadow-sm shadow-black/20"><Loader2 className="h-5 w-5 animate-spin" /></div>
          <div><p className="text-sm font-semibold tracking-tight text-black">Verifying BuyMesho access</p><p className="mt-1 text-xs text-black/60">Preparing Ticket Validator...</p></div>
        </div>
      </div>
    );
  }
  ...
  const handleNavChange = (tab: 'events' | 'scan' | 'attendees') => {
    if (tab === currentTab) {
      if (tab === 'events') setViewState('list');
      else if (tab === 'scan') { handleDismissResult(); window.scrollTo({ top: 0, behavior: 'smooth' }); }
      else { setSelectedTicketForDetail(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  ...
  const workspaceNeedsEvent = (currentTab === 'scan' || currentTab === 'attendees') && !selectedEvent;
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] font-sans text-zinc-900 antialiased selection:bg-indigo-100 selection:text-indigo-900">
      ...
      {syncToastMessage && <div className="fixed left-1/2 top-[76px] z-50 flex -translate-x-1/2 z-50 items-center gap-2 rounded-full border border-emerald-200 bg-white px-3.5 py-2 text-[11px] font-semibold text-emerald-700 shadow-lg" role="status"><CheckCircle2 className="h-3.5 w-3.5" /><span>{syncToastMessage}</span></div>}
      ...
      <main className="mx-auto min-h-[calc(100vh-64px)] w-full max-w-5xl px-3 pb-24 pt-3 transition-all duration-200 sm:px-5 sm:pb-28 sm:pt-5">
        ...
        {currentTab === 'scan' && workspaceNeedsEvent && (
          <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center px-6 text-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-100 text-slate-700"><ScanLine className="h-5 w-5" /></div>
            <h2 className="mt-4 text-base font-semibold text-slate-900">Select an event first</h2>
            <p className="mt-2 text-xs leading-5 text-slate-500">Choose an event from Events before opening the scanner.</p>
            <button type="button" onClick={() => setCurrentTab('events')} className="mt-5 min-h-11 rounded-xl bg-slate-950 px-5 py-3 text-xs font-semibold text-white">Go to Events</button>
          </div>
        )}
        ...
        {currentTab === 'attendees' && workspaceNeedsEvent && (
          <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center px-6 text-center">
            <h2 className="text-base font-semibold text-slate-900">Select an event first</h2>
            <p className="mt-2 text-xs leading-5 text-slate-500">Choose an event from Events before opening attendees.</p>
            <button type="button" onClick={() => setCurrentTab('events')} className="mt-5 min-h-11 rounded-xl bg-slate-950 px-5 py-3 text-xs font-semibold text-white">Go to Events</button>
          </div>
        )}
      </main>
