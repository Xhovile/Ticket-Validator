...
function getString(...values: Array<unknown>): string { for (const value of values) { if (typeof value === 'string' && value.trim()) return value.trim(); if (typeof value === 'number' && Number.isFinite(value)) return String(value); } return ''; }
...
export type ValidatorEventRecord = {
  id: string; title: string; organizerName: string; eventDate: string; startTime: string; venue: string; location: string;
  ticketLink: string | null; status: string; ticket_count?: number; event_type?: string; description?: string;
  ticket_mode?: string; ticket_price?: number | null; contact_whatsapp?: string | null; poster_alt?: string | null;
  spec_values?: Record<string, unknown>; creator_uid?: string | null; created_at?: string; updated_at?: string;
};
...
export function mapValidatorEvent(event: ValidatorEventRecord, organizerId = ''): EventItem {
  ...
  return {
    id: event.id, name: getString(event.title, 'Untitled Event'), organizerId: getString(event.creator_uid, organizerId), organizerName: getString(event.organizerName),
    date: getString(event.eventDate), startTime: getString(event.startTime), venue: getString(event.venue), city: getString(event.location),
    bannerImage: getValidatorEventImageUrl(posterSource), state: getEventState(getString(event.eventDate), getString(event.startTime), getString(event.status)),
    totalTicketsSold: Number(event.ticket_count) || 0, checkedInCount: 0, category: getString(event.event_type, 'Event'), gates: ['Main Gate'],
    description: getString(event.description), ticketMode: getString(event.ticket_mode), ticketPrice: event.ticket_price == null ? null : Number(event.ticket_price),
    ticketLink: event.ticketLink ?? null, contactWhatsapp: event.contact_whatsapp ?? null, createdAt: event.created_at, updatedAt: event.updated_at,
  };
}
