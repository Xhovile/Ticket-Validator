...
async function requireFreshFirebaseToken(forceRefresh = false) {
  if (!auth.currentUser) throw Object.assign(new Error('No authenticated Firebase user.'), { status: 401 });
...
function buildApiError(response: Response, payload: unknown) {
  const error = payload && typeof payload === "object" && "error" in payload
    ...
    : "Request failed";
  return Object.assign(new Error(error), { status: response.status, payload });
}
...
async function fetchJson<T>(path: string, _unusedToken?: string, init?: RequestInit): Promise<T> {
  ...
  try { result = await requestJson<T>(path, effectiveToken, init); }
  catch (error) { throw Object.assign(new Error(`Unable to reach BuyMesho Validator API: ${error instanceof Error ? error.message : String(error)}`), { status: 0, cause: error }); }
  if (result.response.status === 401) {
    effectiveToken = await requireFreshFirebaseToken(true);
    try { result = await requestJson<T>(path, effectiveToken, init); }
    catch (error) { throw Object.assign(new Error(`Unable to reach BuyMesho Validator API: ${error instanceof Error ? error.message : String(error)}`), { status: 0, cause: error }); }
    if (result.response.status === 401) { await signOutValidator().catch(() => undefined); throw buildApiError(result.response, result.payload); }
  }
  if (!result.response.ok) {
    const error = buildApiError(result.response, result.payload);
    if (error.status === 403) error.status = VALIDATOR_ACCESS_DENIED_STATUS;
    throw error;
  }
...
export async function exchangeValidatorSession(token: string) {
  try {
    ...
    if (!payload || typeof payload.customToken !== "string" || !payload.customToken) {
      throw Object.assign(new Error("BuyMesho returned an invalid Validator session response."), { status: 502, payload });
    }
    return payload as SessionExchangeResponse;
  } catch (error) {
    if (error && typeof error === 'object' && 'status' in error) throw error;
    throw Object.assign(new Error(`Unable to reach BuyMesho Validator API: ${error instanceof Error ? error.message : String(error)}`), { status: 0, cause: error });
  }
}
