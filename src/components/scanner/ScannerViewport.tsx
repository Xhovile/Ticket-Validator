...
export const ScannerViewport: React.FC<{
  ...
  zoomOpen: boolean;
  onToggleZoom: () => void;
}> = ({ regionId, cameraActive, cameraError, isStartingCamera = false, onRetryCamera, zoomLevel, zoomOpen, onToggleZoom }) => (
  <div className="relative mx-auto aspect-square w-full overflow-hidden rounded-[1.25rem] border border-slate-800 bg-[#020617] shadow-lg shadow-slate-950/10">
    ...
    <div className="absolute left-3 right-3 top-3 flex items-start justify-between">
      <div className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-black/55 px-2.5 py-1.5 text-[10px] font-medium text-white backdrop-blur-md">
        <span className={`h-1.5 w-1.5 rounded-full ${cameraActive ? 'bg-emerald-400' : isStartingCamera ? 'bg-amber-400 animate-pulse' : 'bg-red-400'}`} />
        <span>{cameraActive ? 'Camera ready' : isStartingCamera ? 'Starting camera' : 'Camera unavailable'}</span>
      </div>
      <button type="button" onClick={() => { onToggleZoom(); soundFX.playClick(); }} className={`flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-[10px] font-semibold text-white shadow-sm backdrop-blur-md transition ${zoomOpen ? 'border-white/20 bg-[#020617]' : 'border-white/10 bg-black/55 hover:bg-black/70'}`} aria-expanded={zoomOpen} aria-label="Camera zoom controls">
    ...
    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 rounded-full border border-white/10 bg-black/55 px-3 py-1.5 text-center text-[10px] font-medium text-white backdrop-blur-md">
      {cameraActive ? 'Position QR code inside the frame' : isStartingCamera ? 'Preparing camera…' : 'Camera needs attention'}
    </div>
    {!cameraActive && (
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#020617]/90 px-6 text-center text-white backdrop-blur-[2px]">
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-slate-200"><Camera className="h-6 w-6 text-white" /></div>
        <p className="mt-4 text-sm font-semibold text-white">{isStartingCamera ? 'Preparing camera' : 'Camera unavailable'}</p>
        <p className="mt-1.5 max-w-[270px] text-[10px] leading-5 text-slate-300">{cameraError || 'Allow camera access, then point the camera at the attendee\'s QR code.'}</p>
        {!isStartingCamera && onRetryCamera && (
          <button type="button" onClick={onRetryCamera} className="mt-4 rounded-lg border border-white/15 bg-white/10 px-3.5 py-2 text-[10px] font-semibold text-white transition hover:bg-white/15">Retry camera</button>
        )}
        <div className="mt-4 inline-flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-[0.14em] text-slate-400"><ScanLine className="h-3 w-3" /> Scanner ready when camera connects</div>
      </div>
    )}
  </div>
);
