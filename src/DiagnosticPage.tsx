...
export default function DiagnosticPage() {
  ...
  const runDiagnostic = async () => {
    ...
    if (!user) { push({ name: 'Firebase authentication', status: 'fail', detail: 'No Firebase user is signed in in this browser.' }); setRunning(false); return; }
    ...
    try {
      const sessionResult = await request('/api/validator/session', token, { method: 'POST', body: JSON.stringify({}) });
      ...
      const safePayload = payload && typeof payload === 'object' ? { ...payload, ...(customTokenReturned ? { customToken: '[REDACTED]' } : {}) } : payload;
      push({ name: 'POST /api/validator/session', status: sessionResult.ok && customTokenReturned ? 'pass' : sessionResult.status === 401 || sessionResult.status === 403 || sessionResult.status === 404 || sessionResult.status >= 500 ? 'fail' : 'warn', detail: `HTTP ${sessionResult.status} in ${sessionResult.ms}ms. ${customTokenReturned ? 'BuyMesho returned a custom session token.' : 'No custom session token was returned.'}`, data: safePayload });
    } catch (error) { push({ name: 'POST /api/validator/session', status: 'fail', detail: error instanceof Error ? error.message : String(error) }); }
